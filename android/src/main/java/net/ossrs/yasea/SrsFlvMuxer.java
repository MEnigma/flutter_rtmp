...
public class SrsFlvMuxer {
    ...
    private void sendFlvTag(SrsFlvFrame frame, boolean historydst) {
        if (frame == null) {
            return;
        }
                    ...
                    while (!mFlvTagCache.isEmpty()) {
                        ...
                        if (frame.isSequenceHeader()) {
                            if (frame.isVideo()) {
                                ...
                                videodts = Math.max(frame.dts, videodts);
                                sendFlvTag(mVideoSequenceHeader, frame.dts < videodts);
                            } else if (frame.isAudio()) {
                                mAudioSequenceHeader = frame;
                                audiodts = Math.max(frame.dts, audiodts);
                                sendFlvTag(mAudioSequenceHeader, frame.dts < audiodts);
                            }
                        } else {
                            if (frame.isVideo() && mVideoSequenceHeader != null) {
                                videodts = Math.max(frame.dts, videodts);
                                sendFlvTag(frame, frame.dts < videodts);
                            } else if (frame.isAudio() && mAudioSequenceHeader != null) {
                                audiodts = Math.max(frame.dts, audiodts);
                                sendFlvTag(frame, frame.dts < audiodts);
                            }
                        }
                    }
